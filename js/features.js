...
(function () {
  ...
  function getRandomFlavorText() {
    const index = Math.floor(Math.random() * FLAVOR_MESSAGES.length);
    return FLAVOR_MESSAGES[index];
  }

  function updateStreakUI(streak) {
    const countEl = document.getElementById('streak-count');
    const unitEl = document.getElementById('streak-unit');
    if (countEl) countEl.textContent = String(streak ?? getStreak());
  ...
  /** Call when a task is marked done (from app.js). */
